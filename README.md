...
Nền tảng đặt tour, khách sạn và vé du lịch trực tuyến, tối ưu cho hệ sinh thái **Netlify**, sử dụng **Supabase (PostgreSQL)** làm cơ sở dữ liệu.
...
- **Chống overbooking:** Row-Level Lock (`SELECT ... FOR UPDATE`) bên trong hàm Postgres RPC
...
- **Tạo booking an toàn qua Supabase RPC**: khóa dòng tồn kho (`FOR UPDATE`) để tránh book trùng khi có nhiều request đồng thời. Có ràng buộc `CHECK (booked_slots <= total_slots)` ở tầng DB.
...
- **`/admin/products`** — Quản lý sản phẩm. Table hiển thị ID, Type, Title, Adult/Child/Infant Price. Nút "Add Product" mở modal, "Edit" / "Delete" thao tác ngay trong table.
- **`/admin/inventory`** — Quản lý tồn kho. Chọn sản phẩm → xem 30 ngày tới (Date, Total Slots, Booked Slots, Available). Chỉnh sửa `total_slots` inline (blur để lưu).
- **`/admin/bookings`** — Quản lý đơn đặt. Filter theo `payment_status` (unpaid/paid/cancelled) và tìm kiếm theo Booking ID / User ID. Nhấn "Details" mở modal chi tiết (items + products), dropdown thay đổi `payment_status`.
...
src/
  lib/
    supabase.ts         # Supabase client (server service_role + browser anon, lazy init)
...
# 3. Trên Supabase SQL Editor, chạy tuần tự 3 file:
...
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | Anon key (public, dùng ở browser) |
...
| `NEXT_PUBLIC_BASE_URL` | URL gốc site (dùng build returnUrl / ipnUrl) |
...
## 6. VNPAY Integration (Supabase JS)

### 6.1. Tạo URL thanh toán (`/api/payment/vnpay-url`)
- Nhận `bookingId`, truy vấn Supabase (`supabaseServer.from("bookings")`).
- Tính secure hash HMAC-SHA512 (sort field alphabetically, sign bằng `VNP_HASH_SECRET`).
- Trả về URL redirect chuẩn VNPAY (amount tính bằg VND nguyên, nhân 100).

### 6.2. Webhook IPN (`/api/payment/vnpay-ipn`)
- Validate `vnp_SecureHash` bắng `verifySecureHash()`.
- Tìm booking qua `vnp_TxnRef`, so sánh số tiền (`vnp_Amount / 100`).
- Nếu `vnp_ResponseCode === '00'` → cập nhật `payment_status = 'paid'`.
- Nếu thất bại → gọi RPC `rollback_booking_inventory(p_booking_id)` để hoàn trả `booked_slots` và đánh dấu `'cancelled'`.
- Luôn trả JSON chuẩn: `{"RspCode":"00","Message":"Confirm success"}`.

---

## 7. Deploy lên Netlify

### 7.1. Kết nối repository
...
### 7.2. Biến môi trường trên Netlify
...
### 7.3. Khởi tạo DB trên Supabase
Trong Supabase dashboard (SQL Editor), chạy 3 file SQL một lần: `0001_init.sql`, `0002_rpc_booking.sql`, `seed.sql`.

### 7.4. Deploy
Mỗi `git push` lên `master` tự động trigger deploy trên Netlify.

---

## 8. Test luồng thanh toán (Sandbox)
